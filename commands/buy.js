...
module.exports = {
    ...
    async execute(message, args, { db }) {
        ...
        const additionalArg = args[1]; // For card protector
        ...
        if (item === 'cardpack') {
            ...
            if (!additionalArg) {
                return message.reply(
                    'Please specify the ID of the card you want to protect.  Example: `!buy cardprotector card_id`'
                );
            }
            const cardIdToProtect = additionalArg;
            if (!db.data.users[userId].inventory.includes(cardIdToProtect)) {
                return message.reply(
