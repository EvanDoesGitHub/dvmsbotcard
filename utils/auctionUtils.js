...
async function sweepExpiredAuctions(client, db) {
  await db.read();
  // console.log('db.data after read:', db.data); // Remove this line
  // console.log('db.data.auctions after read:', db.data?.auctions); // Remove this line

  // Ensure auctions is always an object (or handle if it's undefined/null)
  db.data.auctions ||= {};
  ...
  for (const auction of Object.values(db.data.auctions)) {
    ...
    if (Date.now() >= auction.end) { // Use auction.end as per your data structure
      auction.ended = true;

      const channel = await client.channels.fetch(auction.owner).catch(() => null); // Assuming owner is the channel ID? Double-check this.
      if (!channel) continue;
      ...
      if (auction.highestBidder) {
        ...
        const card = auction.item; // Use auction.item as per your data structure
        seller.balance += auction.highestBid;
        winner.balance -= auction.highestBid;
        winner.inventory.push(card);
        ...
        const embed = new EmbedBuilder()
          .setTitle('🎉 Auction Ended')
          .setDescription(`Card **${card.title || card.cardId}** sold to <@${auction.highestBidder}> for **${auction.highestBid}₩**.`) // Added fallback for title
          .setColor('Green');

        channel.send({ embeds: [embed] });
      } else {
        const seller = db.data.users[auction.owner] ||= { inventory: [], balance: 0 };
        seller.inventory.push(auction.item); // Use auction.item

        const embed = new EmbedBuilder()
          .setTitle('⌛ Auction Ended')
          .setDescription(`No bids for **${auction.item.title || auction.item.cardId}**. It was returned to <@${auction.owner}>.`) // Added fallback for title
          .setColor('Red');
